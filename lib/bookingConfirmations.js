...
function hotelFromNightStays(itinerary, dayDates) {
  ...
  return (itinerary.nightStays || [])
    .flatMap((stay, index) => {
      if (!stay.hotelName && !stay.hotelId) return []
      ...
      const primary = {
        ...
        quotedRoomPricePerNight: roomPricePerNight,
      }

      const rows = [primary]

      // The client returns to this hotel later in the trip. It stays ONE
      // hotel card (not the same hotel twice) — the return dates are shown
      // on it exactly as entered in the itinerary. The room line's Nights is
      // already the TOTAL for the whole stay (what the itinerary's night-count
      // check and price use), so nights/cost are NOT topped up with the return
      // visit a second time. Same room type/rate is assumed unless Operations
      // changes it on confirm.
      if (stay.hasReCheckIn && stay.reCheckIn) {
        primary.returnCheckIn = stay.reCheckIn
        primary.returnCheckOut = stay.reCheckOut || null
      }
...
export function mergeConfirmations(source, saved) {
  ...
  return source.map((item) => {
    ...
    return {
      ...
      // Only when the itinerary has a re-check-in for this hotel; Operations'
      // saved edit wins, same as check-in/out above.
      returnCheckIn: item.returnCheckIn ? existing?.returnCheckIn || item.returnCheckIn : null,
      returnCheckOut: item.returnCheckIn ? existing?.returnCheckOut || item.returnCheckOut || null : null,
