...
function hotelFromNightStays(itinerary, dayDates) {
  ...
  return (itinerary.nightStays || [])
    .flatMap((stay, index) => {
      ...
      // The client returns to this hotel later in the trip — its own
      // confirmation row, with the hand-entered return dates. Same room
      // type/rate is assumed unless Operations changes it on confirm.
      if (stay.hasReCheckIn && stay.reCheckIn) {
        const returnNights = nightsBetween(stay.reCheckIn, stay.reCheckOut)
        rows.push({
          ...primary,
          key: `${stay.hotelId || stay.hotelName}-${index}-return`,
          checkIn: stay.reCheckIn,
          checkOut: stay.reCheckOut || null,
          nights: returnNights || null,
          quotedPrice:
            roomPricePerNight && roomCount && returnNights
              ? roomPricePerNight * roomCount * returnNights
              : null,
        })
      }
