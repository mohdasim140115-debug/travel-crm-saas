...
function drawOceanHotelsAndPricingByTier(doc, { hotels, hotelBuffers, itinerary }, y) {
  ...
  ordered.forEach(({ category, total: catTotal }) => {
    y = drawOceanTierHeader(doc, `${tierLabel(category)} Package`, category, y)
    ...
    const isHigh = category === 'high'
    y = drawOceanPillRow(doc, y, {
      label: `Total Package Cost — ${tierLabel(category)}`,
...
function drawEmeraldHotelsAndPricingByTier(doc, { hotels, hotelBuffers, itinerary }, y) {
  ...
  ordered.forEach(({ category, total: catTotal }) => {
    y = drawEmeraldTierHeader(doc, `${tierLabel(category)} Package`, y)
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(12.5)
      .text(`TOTAL PACKAGE COST — ${tierLabel(category).toUpperCase()}`, EM + 22, y + 14, { width: ECW - 44 })
    doc
      .fillColor(EMERALD.white)
      .font('Times-Bold')
      .fontSize(21)
